...
class CouponTemplAction extends CommonAction{
    ...
    public function doCreate(){
        ...
        $ext_limit_value = I('ext_limit_value','');
        ...
        if($lifetime > 0 && !empty($description)){
            ...
            if($limit_value > 0){
                ...
                //扩展限制
                if(!empty($ext_limit_value))
                {
                    $ext_limit_array = $arr = explode(",",$ext_limit_value);
                    if(!empty($ext_limit_array))
                    {
                        $data['ext_limit_value'] = json_encode($ext_limit_array);
                    }
                }
            }
