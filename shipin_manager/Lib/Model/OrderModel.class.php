...
class OrderModel extends RelationModel {
    ...
    /**
     *  为优化查询性能，只提供按订单状态,订单号，团单号, 后台修改时间，
     *  运单号，分站 ，进行查询
     *  chrisying 2016-1-12
     * @param int $status
     ...
     * @return array
     */
    public  function getNewOrdersList($status = 0, $whereArr = array()) {
...
//        }
        ...
        $strPage = $page->show();
        $orderInfo = $this->relation(true)->field(C('DB_PREFIX') . 'order.*')->where($whereArr)->relation(true)
...
//            $item['order_status_name'] = $manageOrderStatusArr[$status];
    ...
    public  function getFetchOrdersList($status = 0, $whereArr = array()) {
        ...
        echo '<br/>=====取出订单====sql==' . $this->getLastSql() . '<br/>';
        ...
        foreach ($orderInfo as $key => $item) {
            ...
            //更新order表has_export 为1
            $where = array('order_name' => $item['order_name']);
            $this->where($where)->save(array('has_export' => 1));
