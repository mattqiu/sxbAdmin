...
//        $cardList = $cardM->where('coupon_tml=88')->limit(0, 1000)->select();

        $where = array();
        $where['groupbuyingtml_id'] = 49;
        $where['create_time'] = array('gt', strtotime('2016-01-27 20:00:00'));
        $where['used_coupon'] = array('neq', '');
        $orderList = $groupbuyingOrderM->where($where)->group('groupbuying_id')->limit(0, 100)->select();
        ...
        $groupbuyingIdArr = array();
        foreach($orderList as $order){
            $groupbuyingIdArr[] = $order['groupbuying_id'];
        }

        $where2 = array();
        $where2['id`'] = array('in', $groupbuyingIdArr);
        $groupbuyingM->where($where2)->select();

        echo '<br/>' . $groupbuyingM->getLastSql() . '<br/>';

//
//        $groupbuyingArr = array();
//        foreach($cardList as $card){
//            $where = array();
//            $where['used_coupon'] = array('like', '%' . $card['id'] . '%');
//            $orderItem = $groupbuyingOrderM->where($where)->group('groupbuying_id')->find();
//            if(!empty($orderItem)){
//                $groupbuyingArr[] = $orderItem;
//            }
//
////            echo '<br/>===实例sql===' . $groupbuyingOrderM->getLastSql() . '<br/>';
//        }
//
//        var_dump($groupbuyingArr);
//        if(empty($groupbuyingArr)){
//            echo '没有需要改的团实例';
//            exit;
//        }
//
//
//        $groupbuyingIdArr = array();
//        foreach($groupbuyingArr as $item){
//            $groupbuyingIdArr[] = $item['id'];
//        }
//
//        $orderWhere = array();
//        $orderWhere['id'] = array('in', $groupbuyingIdArr);
//        $groupbuyingM->where($orderWhere)->select();
//        echo '<br/>' . $groupbuyingM->getLastSql();
    }
}